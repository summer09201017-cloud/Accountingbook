...
function renderList(entries) {
  ...
  for (const entry of entries) {
    ...
    let notePart = entry.note ? `・${entry.note}` : "";
    ...
    meta.innerHTML = `${formatDate(entry.date)}・${icon} ${entry.category}・${account}${notePart}${recurringPart}`;
...
function createBudgetReportBody(spent) {
  ...
  if (!budget || budget <= 0) {
    ...
    if (els.budgetSprite) {
      if (ratio >= 1) els.budgetSprite.textContent = "😭";
      else if (ratio >= 0.8) els.budgetSprite.textContent = "😰";
      else if (ratio >= 0.5) els.budgetSprite.textContent = "🙂";
      else els.budgetSprite.textContent = "🤩";
    }
  ...
  if (els.customColorInput) {
    els.customColorInput.addEventListener('change', (e) => {
      ...
      if (els.themeFeedbackText) {
        els.themeFeedbackText.textContent = \�w�M�Φۭq��X \�A�N�󭫷s���J��ͮ�\;
...
function checkVersionUpdate() {
  ...
  if (lastVersion !== currentVersion) {
    showToast(\?? ��s���\�I�ثe���� \�A�s�W�F���ҡB�ۭq�D�D��P�O�b�p���F���\��I\, { duration: 5000 });
...
async function updateStorageWarning() {
  ...
  try {
    ...
    els.storageUsageText.textContent = \�w�ϥ� \ MB / \ MB\;
    const pct = Math.min(ratio * 100, 100);
...
function setupPinLock() {
  ...
  if (els.pinLockToggleBtn) {
    els.pinLockToggleBtn.textContent = savedPin ? '?? ���� PIN �X��' : '?? �ҥ� PIN �X��';
    els.pinLockToggleBtn.addEventListener('click', () => {
      if (savedPin) {
        localStorage.removeItem('ledger_pin');
        els.pinLockToggleBtn.textContent = '?? �ҥ� PIN �X��';
        showToast('�w���� PIN �X��');
      } else {
        isSettingPin = true;
        settingPinStep = 0;
        ...
        newPinBuffer = '';
        els.pinLockOverlay.hidden = false;
        document.getElementById('pinLockTitle').textContent = '�г]�w 4 ��� PIN �X';
        updatePinDots(0);
      }
    });
  }
  ...
  function handlePinSubmit() {
    if (isSettingPin) {
      if (settingPinStep === 0) {
        ...
        currentInput = '';
        settingPinStep = 1;
        document.getElementById('pinLockTitle').textContent = '�ЦA����J�H�T�{';
        updatePinDots(0);
      } else {
        if (currentInput === newPinBuffer) {
          ...
          isSettingPin = false;
          els.pinLockOverlay.hidden = true;
          els.pinLockToggleBtn.textContent = '?? ���� PIN �X��';
          showToast('�w�]�w PIN �X��');
        } else {
          currentInput = '';
          settingPinStep = 0;
          document.getElementById('pinLockTitle').textContent = '�K�X���@�P�A�Э��s�]�w';
          updatePinDots(0);
          if (els.pinError) {
      ...
      if (currentInput === savedPin) {
        ...
        updatePinDots(0);
        if (els.pinError) {
          els.pinError.textContent = '�K�X���~�A�Э���';
          els.pinError.hidden = false;
        }
      }
    }
...
function setupNlpInput() {
  ...
  els.nlpInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      ...
      let note = text.replace(amount, '').replace(/�Q��|�e��/g, '').trim();
      ...
      updateAmountPreview();
      updateHint('�w�z�L�۵M�y���ѪR�a�J�A�нT�{��e�X�C', false);
      els.nlpInput.value = '';
    }
  });
}
...
const goldThemeHtml = '<button type="button" class="theme-swatch" data-color="gold" style="background: linear-gradient(135deg, #FFD700, #FDB931, #9E7B15);" aria-label="�aģ�S�� (30�Ѧ��N����)"></button>';
...
checkAchievements = function() {
  ...
  if (currentStreak >= 3 && !state.achievementsUnlocked.has("nospend_3")) {
    state.achievementsUnlocked.add("nospend_3");
    persistAchievements();
    showAchievementPopup("nospend_3", "�s�� 3 �ѵL���O");
  }
  ...
  if (currentStreak >= 30) {
    if (!state.achievementsUnlocked.has("nospend_30")) {
      ...
      persistAchievements();
      showAchievementPopup("nospend_30", "�aģ�S���I�s�� 30 �ѵL���O");
    }
    // Unlock Gold Theme
